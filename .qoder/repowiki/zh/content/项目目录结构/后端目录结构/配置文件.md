...
`application.properties` 是 Spring Boot 应用的核心配置文件，位于 `backend/src/main/resources/` 目录下，定义了服务端口、数据库连接、JPA 行为、SQL 初始化、日志和 JSON 序列化等关键参数。该文件与 `schema.sql` 和 `test-data.sql` 配合，实现数据库的自动创建和数据初始化。
